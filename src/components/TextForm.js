...
export default function TextForm(props) {
  ...
  <h1 className={`my-3 text-${props.mode==='light' ? 'dark' : 'light'}`}>{props.value}</h1>
  ...
  <button className="btn btn-primary my-3" onClick={onUpClick}>{props.btn1}</button>
  <button className="btn btn-primary my-3 mx-3" onClick={onLowClick}>{props.btn2}</button>
  <button className="btn btn-primary my-3 mx-3" onClick={onClear}>Clear Text</button>
</div>
<div className={`container  text-${props.mode==='light' ? 'dark' : 'light'}`}>
    <h2>Your text summary </h2>
    <p>{text.split(" ").length} words and {text.length} charac</p>
